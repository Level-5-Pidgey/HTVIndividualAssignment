...
namespace HTVIndividualAssignment
{
    public partial class MainScreen : Form
    {
        ...
        public MainScreen(string dbFilePath)
        {
            InitializeComponent();
            _dbFilePath = dbFilePath;

            //Establish connection to database
            databaseConn = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=" + _dbFilePath + ";Integrated Security=True");
        ...
        private bool Check_Value_Entry()
        {
            ...
            if (this.FirstNameText.Text.Trim() == "" || !Regex.IsMatch(this.PhoneNoText.Text.Trim(), "[0-9]")) //Real names should not have numerical values!
            {
                result = false;
            }
            if (this.LastNameText.Text.Trim() == "" || !Regex.IsMatch(this.PhoneNoText.Text.Trim(), "[0-9]")) //Real names should not have numerical values!
            {
                result = false;
            }
        ...
        private void Update_Database_Window()
        {
            ...
            {
                MessageBox.Show("Update of database window failed: " + ex.Message);
            }
        }
